...
public class ParserTest extends BaseMockTest {
    ...
    public void testSource() {
        ...
        assertParser(Parsers.tokenType(Integer.class, "int").from(INTEGER, Scanners.WHITESPACES).source(), "123", "123");
    }
    ...
    public void testTimes_range() {
        ...
        assertListParser(isChar('a').step(0).next(isChar('b')).times(1, 2).followedBy(isChar('a')), "aba", 'b');
    ...
    public void testMany1() {
        ...
        assertListParser(isChar('a').step(0).next(isChar('b')).many1().followedBy(isChar('a')), "aba", 'b');
    ...
    public void testSepEndBy1() {
        ...
        assertListParser(INTEGER.sepEndBy1(COMMA.step(0).next(COMMA)).followedBy(COMMA), "1,", 1);
        ...
        assertFailure(INTEGER.followedBy(COMMA).sepEndBy1(COMMA), "1,,1", 1, 5, ", expected, EOF encountered.");
        // 0 step partial delimited consumption
        assertListParser(INTEGER.step(0).followedBy(COMMA).sepEndBy1(COMMA).followedBy(string("1")), "1,,1", 1);
    }
    ...
    public void testSepEndBy() {
        ...
        // 0 step partial delimiter consumption
        assertListParser(INTEGER.sepEndBy(COMMA.step(0).next(COMMA)).followedBy(COMMA), "1,", 1);
        // partial delimiter consumption
        ...
        // partial consumption on delimited.
        assertFailure(INTEGER.followedBy(COMMA).sepEndBy(COMMA), "1,,1", 1, 5, ", expected, EOF encountered.");
        // 0 step partial delimited consumption
        assertListParser(INTEGER.step(0).followedBy(COMMA).sepEndBy(COMMA).followedBy(string("1")), "1,,1", 1);
    }
    ...
    @Mock
    Map2<Integer, Integer, Integer> binaryOp;
    ...
    public void testFrom() {
        ...
        assertFailure(Terminals.CharLiteral.PARSER.from(constant(Arrays.<Token>asList())), "", 1, 1, "character literal expected, EOF encountered.");
    ...
    public void testFrom_throwsOnScanners() {
        assertFailure(string("foo").from(constant(Arrays.asList(new Token(0, 3, "foo")))), "foo", 1, 1, "Cannot scan characters on tokens.");
        assertFailure(isChar('f').from(constant(Arrays.asList(new Token(0, 1, 'f')))), "f", 1, 1, "Cannot scan characters on tokens.");
    }
    ...
    private static void assertListParser(Parser<? extends List<?>> parser, String source, Object... expected) {
